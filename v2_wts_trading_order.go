...
func (c *V2WtsTradingOrder) CreateDirect(ctx context.Context, orderReq *OrderCreateDirectRequest) (out *OrderCreateDirectResponse, ferr error) {
	if orderReq.OrderPriceType == "" {
		orderReq.OrderPriceType = "00"
	}

	var ticker string
	switch orderReq.TradeType {
	case TradeTypeSell, TradeTypeBuy:
	default:
	...
	if err != nil {
		slog.Error(ferr.Error(), slog.String("op", string(orderReq.TradeType)), slog.String("stockCode", orderReq.StockCode), slog.Float64("price", orderReq.Price), slog.Float64("qty", orderReq.Quantity), slog.String("ticker", ticker))
		return nil, err
	}
	...
	defer func() {
		if ferr != nil {
			...
			slog.Info("created order", slog.String("op", string(orderReq.TradeType)), slog.String("stockCode", orderReq.StockCode), slog.Float64("price", orderReq.Price), slog.Float64("qty", orderReq.Quantity), slog.String("ticker", ticker))
		}
	}()
